...
class ProfileView(RetrieveUpdateAPIView):
    ...
    #********     lookup_field = 'pk' -- kullanmak yerine bu fonksiyon ile bir obje döndürdük.
    def get_object(self):
...
class UpdatePassword(APIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if not self.object.check_password(old_password):         # eski şifre mevcut şifre ile uyuşuyor mu
                return Response({"old_password": "wrong password"}, status=status.HTTP_400_BAD_REQUEST)

            self.object.set_password(serializer.data.get("new_password"))
            self.object.save()

            update_session_auth_hash(request, self.object) # bu kod sayesinde şifre değiştirildikten sonra oturumdan atılmıyoruz.
